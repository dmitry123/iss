...
namespace libface
{
	...
	void FisherCore::calulateFisherStats()
	{
		...
		cvConvert(img, img2);
		CvMat* row = LibFaceUtils::transpose(LibFaceUtils::reshape(img2));
		...
		CvMat* row2 = LibFaceUtils::transpose(LibFaceUtils::reshape(img2_training));

		CvMat* T = LibFaceUtils::combine(row, row2);

		img = cvLoadImage("src/TrainDatabase/5.jpg", 0); //0 - grayscale
		img2 = cvCreateMat(img->height, img->width, CV_32FC1);
		cvConvert(img, img2);
		row = LibFaceUtils::transpose(LibFaceUtils::reshape(img2));

		T = LibFaceUtils::combine(T, row);
		...
		CvMat* V_PCA_t = LibFaceUtils::transpose(V_PCA);
		...
		for (i = 0; i < P; i++)
		{
			...
			{
				ProjectedImages_PCA = LibFaceUtils::combine(ProjectedImages_PCA, temp);
			}
		}
		printf("DONE\n");
